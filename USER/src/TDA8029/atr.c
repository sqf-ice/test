...
//  if ( HAL_GPIO_ReadPin(SC_OFF_GPIO_PORT, SC_OFF_PIN) != SC_OFF_ACTIVE )
//  {
//    /* power off smartcard */
//    HAL_GPIO_WritePin(SC_RESET_GPIO_PORT, SC_RESET_PIN, GPIO_PIN_RESET);
//    /* wait for card insert */
//    while (HAL_GPIO_ReadPin(SC_OFF_GPIO_PORT, SC_OFF_PIN) != SC_OFF_ACTIVE)
//    {}
//  }

//  /* power on smartcard if not powered */
//  if (HAL_GPIO_ReadPin(SC_CMDVCC_GPIO_PORT, SC_CMDVCC_PIN) != SC_CMDVCC_UP)
//  {
//    SC_POWER(SC_CMDVCC_UP);
//  }

//  SC_HW_Init(protocol->pdevice);
